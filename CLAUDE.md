...
**Current stage**: MVP code exists across the Mini Program, NestJS backend, and voice gateway. Repository builds and voice unit tests pass, but real WeChat identity, database migrations, external voice/payment/push integrations, and production deployment are not yet verified. Fallback behavior varies by API; do not assume every write succeeds offline.
...
| [`docs/DEV_PROGRESS.md`](docs/DEV_PROGRESS.md) | Evidence-based current status, known issues, and next steps |
...
- Match is generated by the backend Tuesday scheduler; the frontend uses a local Tuesday rule and `TEST_MODE = true` only when the backend request falls back
